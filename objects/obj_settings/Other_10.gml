...
if(instance_exists(gjtext)){
instance_destroy(gjtext)}
...
gjtext=instance_create_depth(70, 200, 0, text_typer)
...
title.text = (prefix + Lang_GetString("str_menu_settings"))
language.text = (prefix + Lang_GetString("str_settings_language"))
language_index.text = (prefix + "{color `"+(global.language>0 ? (choice=1 ? "yellow" : "white") : "gray")+"`}<{color `"+(choice=1 ? "yellow" : "white")+"`} "+Lang_GetString("str_language_name")+" {color `"+(global.language<LANGUAGE.MAX-1 ? (choice=1 ? "yellow" : "white") : "gray")+"`}>")
size.text = (prefix + Lang_GetString("str_settings_window_size"))
fullscreen_obj.text = (prefix + Lang_GetString("str_settings_fullscreen"))
if(os_type=os_android||os_type=os_ios||os_type=os_winphone||os_browser=!browser_not_a_browser){
fullscreen_obj_index.text = (prefix + Lang_GetString("str_settings_n/a"))}else{
...
size_index.text = (prefix + Lang_GetString("str_settings_n/a"))}else{
size_index.text = (prefix + window_size_text)}
gjtext.text = (prefix + Lang_GetString("str_settings_gamejolt"))
volume.text=(prefix + Lang_GetString("str_settings_volume"))
border_obj.text = (prefix + Lang_GetString("str_settings_border"))
border_obj_index.text = (prefix + border_text)
backtext.text = (prefix + Lang_GetString("str_settings_save_back"))

if(choice=1){
language.text = (prefix + "{color `yellow`}" + Lang_GetString("str_settings_language"))
language_index.text = (prefix + "{color `"+(global.language>0 ? (choice=1 ? "yellow" : "white") : "gray")+"`}<{color `"+(choice=1 ? "yellow" : "white")+"`} "+Lang_GetString("str_language_name")+" {color `"+(global.language<LANGUAGE.MAX-1 ? (choice=1 ? "yellow" : "white") : "gray")+"`}>")}

if(choice=2){
size.text = (prefix + "{color `yellow`}" + Lang_GetString("str_settings_window_size"))
if(os_type=os_android||os_type=os_ios||os_type=os_winphone||fullscreen=2||Border_IsEnabled()=true){
size_index.text = (prefix + Lang_GetString("str_settings_n/a"))
}else{
size_index.text = (prefix + "{color `yellow`}" + window_size_text)}}

if(choice=3){
fullscreen_obj.text = (prefix + "{color `yellow`}" + Lang_GetString("str_settings_fullscreen"))
if(os_type=os_android||os_type=os_ios||os_type=os_winphone){
fullscreen_obj_index.text = (prefix + Lang_GetString("str_settings_n/a"))
}else{
fullscreen_obj_index.text = (prefix + "{color `yellow`}" + fullscreen_text)}}

if(choice=4){
gjtext.text = (prefix + "{color `yellow`}"+Lang_GetString("str_settings_gamejolt"))}

if(choice=5){
volume.text = (prefix + "{color `yellow`}"+Lang_GetString("str_settings_volume"))}

if(choice=6){
border_obj.text = (prefix + "{color `yellow`}"+Lang_GetString("str_settings_border"))
border_obj_index.text = (prefix + "{color `yellow`}" + border_text)}

if(choice=7){
backtext.text = (prefix + "{color `yellow`}"+Lang_GetString("str_settings_save_back"))}
...
if(LOCK_LANGUAGE!=-1){
	...
	language = instance_create_depth(70, 80, 0, text_typer)
	language_index = instance_create_depth(450, 80, 0, text_typer)
	language_index.text = (prefix + "{color `gray`}< {color `"+(choice=1 ? "yellow" : "white")+"`}"+Lang_GetString("str_language_name")+"{color `gray`} >")
	language.text = (prefix + "{color `"+(choice=1 ? "yellow" : "white")+"`}"+Lang_GetString("str_settings_language_locked"))
}